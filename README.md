...
O atacante se conecta à mesma rede Wi-Fi que o ESP32 (ou cria uma rede clone).

Captura o tráfego HTTP usando ferramentas como Wireshark, tcpdump ou mitmproxy.

Interpreta facilmente as URLs, já que tudo está em texto plano (/?led=on).

O atacante pode modificar requisições ou reenviá-las à vontade para alterar o comportamento do sistema.

Caso esteja usando um proxy malicioso, pode alterar comandos antes que cheguem ao ESP32.
...
Uma vez que o sistema é simples e não apresenta tanta aplicabilidade no dia a dia, isso reduz os interesses de uma possível interceptação, entretanto o problema pode se agravar devido a exposição das credenciais do wi-fi

**ATAQUE 2 - Acesso Não Autorizado ao Endpoint /login**

Passo a passo do ataque:

O atacante descobre o endpoint /login analisando o tráfego HTTP ou testando URLs comuns.

Envia requisições diretamente ao endpoint usando navegador, curl ou ferramentas como Postman.

Como não há autenticação real, qualquer pessoa consegue acessar o endpoint.

O atacante força estados do sistema enviando parâmetros arbitrários (ex.: /login?auth=1).

Caso haja funções sensíveis atrás desse endpoint, ele consegue acioná-las sem qualquer controle.
...
A avaliação do projeto ESP32 Web Server evidencia vulnerabilidades críticas decorrentes do uso de HTTP sem criptografia e da ausência de autenticação. Esses fatores permitem que atacantes explorem o sistema com facilidade, principalmente por meio de dois vetores: a interceptação e modificação de dados e o acesso não autorizado ao dispositivo. A primeira falha possibilita que o tráfego seja capturado e alterado em redes compartilhadas, enquanto a segunda permite que qualquer indivíduo na mesma rede assuma controle total do ESP32 apenas enviando requisições simples.

Os impactos potenciais são elevados, sobretudo quando o dispositivo controla atuadores físicos, como relés, motores ou sistemas de segurança. Nessas situações, um ataque pode comprometer a integridade, a disponibilidade e até a segurança operacional do ambiente. Diante da alta probabilidade de exploração e do risco associado, a adoção de medidas de mitigação torna-se indispensável. Entre as ações recomendadas estão a implementação de autenticação robusta, o uso de criptografia (HTTPS/TLS ou VPN), o isolamento adequado da rede, além de logs, auditorias e mecanismos de limitação de requisições.
